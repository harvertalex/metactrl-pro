  ...
  const PRESETS = {
    ...
    gambling_plus3: {
      ...
      _rules: [
        ...
        'TurnOff With Expensive Leads and No Registrations or Purchase',
        'TurnOff Without Registrations',
        'TurnOff With Expensive Registrations',
        'TurnOff Without Purchases',
        'TurnOff With Expensive Purchases',
        'TurnOff High Impressions No Purchases',
        'TurnOff Daily Budget Exhaustion',
        ...
        'TurnOn If Cheap Registration (CPA)',
        'TurnOn If Cheap Purchase (CPP)',
        'TurnOn If Clicks Present (>0)',
        'TurnOn If Leads Present (>0)',
        'TurnOn If Registrations Present (>0)',
        'TurnOn If Purchases Present (>0)',
        'TurnOn by Name at Time',
        'TurnOff by Name at Time',
        'Kill Switch: TurnOff All at Time',
...
async function runGenerator(ctx, log = (() => {}), onProgress = (() => {})) {
  ...
  /* ------- NEW: Budget Boost % after N leads (1×/day, check every 30 min) ------- */
  if (selectedRules.includes('Budget: Boost % after N leads with good CPL')) {
    if (artype === 'AD') {
      ...
      log(`⚠️ Budget Boost % after N leads — skipped for ${artype}: budget changes apply to CAMPAIGN only.`, 'warning');
    } else {
...
const CM_METRICS = [
  { key:'hook',     name:'Hook Rate (Video)',         desc:'Attention grab in the first 3 seconds',          bands:'good 30%+ · ok 20–30% · bad <20%',          formula:'actions:video_view / impressions',                      format:'PERCENT', ok:true },
  { key:'hold',     name:'Hold Rate (Video)',         desc:'Retention after the hook',                       bands:'good 50%+ · ok 35–50% · bad <35%',          formula:'video_thruplay_watched_actions:video_view / actions:video_view',   format:'PERCENT' },
  { key:'shook',    name:'Static Hook Rate',          desc:'How much the image stops the scroll (thumbstop)', bands:'good 1.5%+ · ok 0.8–1.5% · bad <0.8%',     formula:'actions:post_engagement / impressions',                  format:'PERCENT' },
  { key:'ictr',     name:'Intent CTR',                desc:'Share who actually clicked off-platform',        bands:'good 1.2%+ · ok 0.7–1.2% · bad <0.7%',      formula:'outbound_clicks:outbound_click / impressions',                   format:'PERCENT' },
  { key:'lpctr',    name:'LP CTR',                    desc:'Click-to-landing efficiency',                    bands:'good 80%+ · ok 60–80% · bad <60%',          formula:'actions:landing_page_view / actions:link_click',         format:'PERCENT' },
  { key:'lpvrate',  name:'LPV Rate',                  desc:'Outbound-click quality (reached the LP)',        bands:'good 80%+ · ok 65–80% · bad <65%',          formula:'actions:landing_page_view / outbound_clicks:outbound_click',     format:'PERCENT' },
  { key:'cplpv',    name:'Cost per LPV',              desc:'Cost of a real site visit',                      bands:'good < target · ok ≈ target · bad > target', formula:'spend / actions:landing_page_view',                            format:'FLOAT' },
  { key:'regrate',  name:'Reg Rate (from LPV)',       desc:'LP → registration rate',                         bands:'good 10%+ · ok 5–10% · bad <5%',            formula:'actions:complete_registration / actions:landing_page_view', format:'PERCENT' },
  { key:'leadrate', name:'Lead Rate (from LPV)',      desc:'LP → lead rate',                                 bands:'good 12%+ · ok 6–12% · bad <6%',            formula:'actions:lead / actions:landing_page_view',               format:'PERCENT' },
  { key:'instrate', name:'Install Rate (from LPV)',   desc:'LP → install rate',                              bands:'good 25%+ · ok 15–25% · bad <15%',          formula:'actions:mobile_app_install / actions:landing_page_view', format:'PERCENT' },
  { key:'epc',      name:'EPC (Earnings Per Click)',  desc:'Earnings per link click',                        bands:'good > breakeven · ok ≈ breakeven · bad < breakeven', formula:'action_values:omni_purchase / actions:link_click',                  format:'FLOAT' },
  { key:'roas',     name:'ROAS (Custom)',             desc:'Return on ad spend',                             bands:'good 1.3+ · ok 1.0–1.3 · bad <1.0',         formula:'action_values:omni_purchase / spend',                               format:'FLOAT' },
  { key:'cei',      name:'Creative Efficiency Index', desc:'Creative efficiency score',                      bands:'good top 25% · ok 25–75% · bad bottom 25%', formula:'actions:post_engagement * outbound_clicks:outbound_click / impressions', format:'PERCENT' },
];
...
function mountPixelManager(container) {
  ...
  const pxState = {
    ...
    pixelMappingError: '', connectedError: '',
  };

  let gqlTemplatePromise = null;
  let connLoadToken = 0;
  ...
  async function gFetchAll(path, params = {}) {
    const rows = []; let next = path; let nextParams = params;
    while (next) {
      const p = await gFetch(next, { params: nextParams });
      rows.push(...(p?.data || []));
      next = p?.paging?.next || ''; nextParams = {};
    }
    return rows;
  }

  /* ---- Internal GraphQL ---- */
  function getModule(name) { try { return typeof globalThis.require === 'function' ? globalThis.require(name) : null; } catch { return null; } }

  function getCurrentUserId() {
    const m = getModule('CurrentUserInitialData');
    for (const c of [m?.USER_ID, m?.ACCOUNT_ID, globalThis.CurrentUserInitialData?.USER_ID]) {
      if (typeof c === 'string' && c.trim()) return c.trim();
    }
    const html = document.documentElement.innerHTML;
    return html.match(/"USER_ID":"(\d+)"/)?.[1] || html.match(/"ACCOUNT_ID":"(\d+)"/)?.[1] || '';
  }

  function getLsdToken() {
    const m = getModule('LSD');
    for (const c of [m?.token, globalThis.LSD?.token, document.querySelector('input[name="lsd"]')?.value]) {
      if (typeof c === 'string' && c.trim()) return c.trim();
    }
    const html = document.documentElement.innerHTML;
    return html.match(/\["LSD",\[\],\{"token":"([^"]+)/)?.[1] || html.match(/name="lsd" value="([^"]+)"/)?.[1] || '';
  }

  async function captureGqlTemplate(businessId) {
    ...
    gqlTemplatePromise = new Promise((resolve, reject) => {
      ...
      XMLHttpRequest.prototype.open = function(method, url) { this.__pmUrl = url; return origOpen.apply(this, arguments); };
      XMLHttpRequest.prototype.setRequestHeader = function(n, v) { hdrs[n] = v; return origSetHeader.apply(this, arguments); };
      XMLHttpRequest.prototype.send = function(body) {
        if (!done && String(this.__pmUrl || '').includes('/api/graphql/')) { finish(resolve, { url: String(this.__pmUrl), body: String(body || ''), headers: { ...hdrs } }); return; }
        return origSend.apply(this, arguments);
      };
      try {
        const req = new AsyncRequest(); req.setMethod('POST'); req.setURI('/api/graphql/');
        req.setData({ doc_id: '0', variables: '{}', __aaid: 0, __bid: businessId || '0' }); req.send();
      } catch (e) { finish(reject, e); return; }
      window.setTimeout(() => finish(reject, new Error('Could not capture internal GraphQL template.')), 3000);
    }).catch(e => { gqlTemplatePromise = null; throw e; });
    return gqlTemplatePromise;
  }

  async function gqlFetch({ name, docId }, variables, businessId) {
    const tmpl = await captureGqlTemplate(businessId);
    const params = new URLSearchParams(tmpl.body);
    const uid = getCurrentUserId();
    if (!uid) throw new Error('Could not determine current user for internal GraphQL.');
    const lsd = getLsdToken() || tmpl.headers['X-FB-LSD'] || '';
    const asbd = tmpl.headers['X-ASBD-ID'] || '359341';
    params.set('av', uid); params.set('__aaid', '0'); params.set('__bid', businessId);
    params.set('fb_api_caller_class', GQL_CALLER); params.set('fb_api_req_friendly_name', name);
  ...
  function mergePixels(pixels, internalAssets) {
    const byName = new Map();
    for (const a of internalAssets) {
      const k = a.name.trim().toLowerCase(); if (!k) continue;
      const cur = byName.get(k) || []; cur.push(a); byName.set(k, cur);
    }
  ...
  async function fetchConnected(bizId, internalAssetId) {
    const payload = await gqlFetch(GQL_CONNECTED, {
      businessID: bizId, fromAssetID: internalAssetId, toAssetType: 'AD_ACCOUNT', searchTerm: null,
    }, bizId);
  ...
  async function loadConnected() {
    ...
    try {
      const assets = await fetchConnected(pxState.businessId, px.internalAssetId);
      if (token !== connLoadToken) return;
      pxState.connectedAssets = assets; pxState.connectedError = '';
    } catch (e) {
      if (token !== connLoadToken) return;
      pxState.connectedError = e.message; pxState.connectedAssets = [];
    } finally {
      if (token !== connLoadToken) return;
      pxState.loadingConnectedAssets = false; render();
    }
  }

  async function loadContext(bizId) {
    pxState.loadingContext = true; pxState.connectedAssets = []; pxState.connectedError = '';
    pxState.pixelMappingError = ''; pxState.businessId = bizId;
    setStatus('info', `Loading BM ${bizId}...`);
    try {
    ...
    } finally { pxState.loadingContext = false; render(); }
  }

  async function loadBusinessesOnly() {
    pxState.loadingBusinesses = true; setStatus('info', 'Loading business managers...');
    try {
      pxState.businesses = await fetchBusinesses();
      setStatus(pxState.businesses.length ? 'success' : 'info', pxState.businesses.length ? 'Select a BM to load its data.' : 'No business managers found.');
  ...
  async function removeConnected(accountId) {
    const px = pxState.pixels.find(p => p.id === pxState.selectedPixelId);
  ...
  function render() {
    ...
    container.querySelectorAll('[data-pxrole]').forEach(el => {
      el.addEventListener('change', async () => {
        ...
        if (role === 'pixel-select')   { pxState.selectedPixelId = el.value; await loadConnected(); }
...
function mountOperations(container) {
  ...
  async function loadOpsAccounts(force) {
    if (OPS_ACCOUNTS_LOADING) return;
    if (!force && OPS_ACCOUNTS_CACHE.length) { setStatus('success', `${OPS_ACCOUNTS_CACHE.length} accounts loaded.`); render(); return; }
    ...
    try {
      ...
      OPS_ACCOUNTS_CACHE = parseAccounts(items);
      setStatus('success', `${OPS_ACCOUNTS_CACHE.length} accounts loaded.`);
    } catch(e) { setStatus('error', e.message); }
    finally { OPS_ACCOUNTS_LOADING = false; render(); }
  }
  ...
  /* ==================== BULK PAUSE / RESUME ==================== */
  async function loadPauseAccounts() { await loadOpsAccounts(); }
  ...
  async function loadZombies() {
    ...
    try {
      if (!OPS_ACCOUNTS_CACHE.length) await loadOpsAccounts();
      const accounts = OPS_ACCOUNTS_CACHE;
